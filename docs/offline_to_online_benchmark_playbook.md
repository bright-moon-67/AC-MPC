...
重标注。最初只取时间顺序前 1,000 个 episode 会偏向探索早期阶段；最终采用
...
Cartpole 最终使用 raw observation 5 维、learned lift 10 维，总 lifted dimension 15。当前
Koopman 训练参数为：
...
谱半径限制与 reference timestep 绑定。迁移任务时必须按实际 control timestep 解析连续时间
衰减，不能把“0.95”当作任何离散步长都相同的常数。
...
4. 无法由 observation 恢复时才训练 reward model，并明确标为 learned-reward 版本。

Cartpole 主实验使用 exact reward oracle。learned reward model只作为 offline/real-world 路线
的消融，不应在能解析 reward 的任务上引入额外误差。
...
| offline updates | 60k development | 0 | 60k development |
| online UTD | 1 | 20 | 20 |
...
Cartpole 当前 capacity-matched structured actor 为：
...
其 trainable actor 约 1.312M 参数；Cal-QL-Raw actor 约 1.060M。两者不是精确同参数量，
但已经处于同一量级。历史 `15->128->240` actor 仅约 33k trainable 参数，不再作为主配置。
...
已经接近高 return，因此当前使用 60k 做快速算法筛选。迁移到新任务时不要自动沿用 60k：
...
| 最终评估 | 10 deterministic episodes | 优先官方协议；建议 10 train seeds × 10 episodes |
...
多 seed 才报告跨 training-seed mean/std/SE/CI。若官方 benchmark 的评估规则不同，官方规则
作为 primary，本项目 10×10 robustness 作为 secondary，不能用 secondary 冒充官方分数。
...
task:
  ...
  action_repeat: null
  episode_steps: null
  reward_contract: exact_oracle | learned_reward
