...
本目录包含从已验证 `manisoft-port` 分支完整迁入的 AC-MPC + ManiSoft 项目，
与外层目标仓库的最新主线代码相互隔离。首次部署、v15e artifact、配套
ManiSoft 提交、环境安装、训练和评估的完整说明见
[README_ACMPC_MANISOFT.md](README_ACMPC_MANISOFT.md)。

不要从外层 AC-MPC 的同名脚本推断本目录行为；本项目命令默认从
`AC-MPC/manisoft_port/` 执行。完成代码克隆后，先运行：

```bash
bash AC-MPC/manisoft_port/scripts/bootstrap_isolated_layout.sh
```

下文保留原始 AC-MPC 基线说明，便于追溯本扩展的历史基础。
