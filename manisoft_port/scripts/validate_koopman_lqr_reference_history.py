...
HISTORY-CONTEXT Koopman model (HistoryDeepKoopman).

Model: ``z_{t+1} = A z_t + B u_t`` (absolute action), but the nonlinear lift
uses a finite history context ``[s[t-H+1:t+1], u[t-H:t]]`` (H=10).  The
physical state is 45-D and the controller outputs the absolute 18-D muscle
activation ``u_t`` directly.

The LQR is solved once in reference error coordinates; the reference is a
constant-action equilibrium, so its context is the reference state/action
repeated over the full history window.  At the exact reference the controller
commands ``u = u_ref``.
"""
...
class HistoryBuffer:
    ...
    Context at time ``t`` is ``[s[t-H+1:t+1], u[t-H:t]]``:
      ...
      - H raw absolute actions (u[t-H] .. u[t-1]; the current action is
        excluded because it is the controller's output).
    """
...
class KoopmanLQRController:
    """Infinite-horizon LQR feedback around a fixed lifted reference for the
    history-context model ``z+ = A z + B u``.

    The controller outputs ``u = u_ref - feedback_scale * K (z - z_ref)``.
    Q is assembled in normalized lifted coordinates over the 45-D physical
    state only; the history context is fixed to the reference's constant
    equilibrium when computing ``z_ref``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        reference_context_action: np.ndarray,
        ...
        self.reference_context_action = np.asarray(
            reference_context_action, dtype=np.float64
        )
        ...
        self.history_steps = int(model.history_steps)
        ...
        # Reference is a constant-action equilibrium, so its context is the
        # reference state/action repeated over the full history window.
        reference_context = self._build_reference_context()
        self.reference_lift = self._lift(self.ref_state, reference_context)
        ...
        # reference in error coordinates.
    ...
    def _build_reference_context(self) -> np.ndarray:
        """Context for the constant reference equilibrium (states and actions
        repeated H times, states normalized, actions raw)."""

        states = np.tile(self.ref_state, (self.history_steps, 1))
        normalized_states = (
            states - self.state_mean.detach().cpu().double().numpy()
        ) / self.state_std_np
        actions = np.tile(self.reference_context_action, (self.history_steps, 1))
        return np.concatenate(
            (normalized_states.reshape(-1), actions.reshape(-1))
        ).astype(np.float32)

    def _lift(self, state: np.ndarray, context: np.ndarray) -> torch.Tensor:
        state_tensor = torch.as_tensor(
            state, dtype=self.model_dtype, device=self.device
        )
        context_tensor = torch.as_tensor(
            context, dtype=self.model_dtype, device=self.device
        )
    ...
    def solve(
        self, state: np.ndarray, context: np.ndarray, previous_action: np.ndarray
    ) -> dict[str, np.ndarray | float]:
        started = time.perf_counter()
        state = np.asarray(state, dtype=np.float64)
        previous_action = np.asarray(previous_action, dtype=np.float64).reshape(-1)
        lifted_error = self._lift(state, context) - self.reference_lift
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="History-context steady-state Koopman-LQR tracking of a "
        "fixed reference"
    )
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--scenario", required=True)
    parser.add_argument("--reference", required=True, help="reference.npz")
    parser.add_argument("--output", default="runs/koopman_lqr_reference_history")
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "Multiply the reference action seen by the controller by this "
            "factor while keeping the target reference state and lifted "
            "reference context unchanged. This simulates feedforward error."
        ),
    )
    parser.add_argument(
        "--no-action-tracking",
    ...
    if not np.isfinite(args.reference_action_scale):
        raise ValueError("reference-action-scale must be finite")
    if min(
        args.state_weight,
        args.control_weight,
        args.max_delta,
    ...
    with np.load(reference_path, allow_pickle=False) as ref:
        ...
        nominal_ref_action = np.asarray(
            ref["reference_action"], dtype=np.float32
        ).reshape(-1)
    if ref_state.shape != (PHYSICAL_DIM,):
        raise ValueError(f"reference_state must be 45-D, got {ref_state.shape}")
    if nominal_ref_action.shape != (ACTION_DIM,):
        raise ValueError(
            f"reference_action must be 18-D, got {nominal_ref_action.shape}"
        )
    if not (
        np.isfinite(ref_state).all() and np.isfinite(nominal_ref_action).all()
    ):
        raise ValueError("reference state/action contains NaN or Inf")
    ref_action = nominal_ref_action * args.reference_action_scale
    ...
    try:
        ...
        initial_nominal_action_dist = float(np.linalg.norm(nominal_ref_action))
        ...
        controller = KoopmanLQRController(
            ...
            reference_context_action=nominal_ref_action,
        ...
        nominal_action_distances = [initial_nominal_action_dist]
        ...
        for step in range(args.steps):
            ...
            control = controller.solve(dynamics_state, context, previous_action)
        ...
        nominal_action_array = np.asarray(
            nominal_action_distances, dtype=np.float32
        )
        ...
        final_nominal_action_dist = float(nominal_action_array[-1])
        ...
        summary = {
            ...
            "device": str(device),
            "controller": "steady_state_koopman_lqr_history",
            ...
            "reference_action": nominal_ref_action.tolist(),
            "controller_reference_action": ref_action.tolist(),
            "reference_context_action": nominal_ref_action.tolist(),
            "reference_action_bias": (
                ref_action - nominal_ref_action
            ).tolist(),
            ...
            "initial_nominal_action_distance": initial_nominal_action_dist,
            "final_nominal_action_distance": final_nominal_action_dist,
        ...
        np.savez_compressed(
            ...
            nominal_action_distance=nominal_action_array,
