...
print("\n===== LQR保持测试 =====")
print("LQR平均末端漂移:", float(controlled.mean()))
print("基线平均末端漂移:", float(baseline.mean()))
print("LQR最终末端漂移:", float(controlled[-1]))
print("基线最终末端漂移:", float(baseline[-1]))
print("LQR最大末端漂移:", float(controlled.max()))
...
if controlled.mean() < baseline.mean():
    ...
    print(f"结论：LQR保持效果优于零动作基线 {improvement:.2f}%")
else:
    print("结论：LQR未优于零动作基线，暂不进行目标跟踪")
