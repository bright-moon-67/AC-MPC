...
# Huge triplet/attempt budgets -> effectively runs continuously.
# Survives SSH disconnect (nohup). Stop anytime with:
#   pkill -f generate_manisoft_waypoint_bank
set -euo pipefail
cd "$(dirname "$0")/.."

OUT=data/processed/manisoft_waypoint_bank_v5_full
SCENARIO=/root/autodl-tmp/ManiSoft/configs/demo_elastica_fast.yaml
N=16
TRIPLETS_PER_SHARD=10000
...
for i in $(seq 0 $((N - 1))); do
  ...
  nohup /root/miniconda3/envs/manisoft/bin/python \
    ...
    --seed "$seed" \
    > "runs/waypoint_bank_v5_${shard}.log" 2>&1 &
done
