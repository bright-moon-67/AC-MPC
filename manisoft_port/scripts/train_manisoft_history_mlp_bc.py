#!/usr/bin/env python
"""Behavior-clone the H=10 History-MLP baseline from MPC demonstrations."""
...
def _evaluate(policy, loader, max_delta: float, device: torch.device) -> dict:
    ...
    for observations, targets in loader:
        ...
        prediction = policy.actor_mean(observations)
        squared_error = (prediction - targets).square()
        squared_error_sum += float(squared_error.sum())
        normalized_error_sum += float((squared_error / (max_delta**2)).sum())
        elements += squared_error.numel()
    mse = squared_error_sum / elements
    return {
        "mse": mse,
        "rmse": mse**0.5,
        "normalized_mse": normalized_error_sum / elements,
    }
...
def parse_args() -> argparse.Namespace:
    ...
    if min(args.epochs, args.batch_size, args.checkpoint_interval) < 1:
        parser.error("Epoch, batch and checkpoint counts must be positive")
    if not 0 < args.validation_fraction < 1:
        parser.error("validation-fraction must lie in (0,1)")
    if min(args.max_delta, args.absolute_action_limit) <= 0:
        parser.error("Action limits must be positive")
    return args
...
def main() -> None:
    ...
    if not checkpoint.is_file() or not dataset_path.is_file():
        raise FileNotFoundError("Metadata checkpoint and expert dataset must exist")
    ...
    policy, _ = make_history_mlp_policy(
        checkpoint,
        device,
    ...
    if args.resume is not None:
        ...
        if resume_payload.get("method") != "history_mlp_bc":
            raise ValueError("Resume checkpoint is not History-MLP behavior cloning")
        ...
        for key in (
            ...
            if resume_payload["runtime"].get(key) != runtime[key]:
                raise ValueError(f"Resume runtime {key} is incompatible")
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            for batch_observations, batch_targets in train_loader:
                ...
                prediction = policy.actor_mean(batch_observations)
                squared_error = (prediction - batch_targets).square()
                ...
                if not torch.isfinite(gradient_norm):
                    raise FloatingPointError("History-MLP BC gradient is NaN or Inf")
                optimizer.step()
                squared_error_sum += float(squared_error.detach().sum())
                elements += squared_error.numel()
                normalized_loss_sum += float(loss.detach())
                gradient_norm_sum += float(gradient_norm.detach())
                batches += 1
            validation = _evaluate(policy, validation_loader, args.max_delta, device)
            elapsed = elapsed_before + time.monotonic() - started
            row = {
                "epoch": epoch,
                "elapsed_seconds": elapsed,
                "train_mse": squared_error_sum / elements,
                "train_rmse": (squared_error_sum / elements) ** 0.5,
                "train_normalized_mse": normalized_loss_sum / batches,
