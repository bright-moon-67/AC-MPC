...
ppo_output="$6"
device="${7:-cuda}"
max_delta="${HISTORY_MLP_MAX_DELTA:-0.001}"
...
if [[ ! -f "${expert_data}" ]]; then
    "${python_bin}" scripts/collect_manisoft_bc_kmpc_expert.py \
        ...
        --reference "${reference}" \
        --output "${expert_data}" \
        --episodes "${HISTORY_MLP_EXPERT_EPISODES:-10}" \
        --episode-steps "${HISTORY_MLP_EPISODE_STEPS:-300}" \
        --horizon "${HISTORY_MLP_EXPERT_HORIZON:-10}" \
        --max-delta "${max_delta}" \
        --device "${device}"
fi
...
if [[ "${bc_complete}" != true ]]; then
    ...
    "${python_bin}" scripts/train_manisoft_history_mlp_bc.py \
        --koopman-checkpoint "${koopman_checkpoint}" \
        --dataset "${expert_data}" \
        --output "${bc_output}" \
        --epochs "${HISTORY_MLP_BC_EPOCHS:-150}" \
        --batch-size "${HISTORY_MLP_BC_BATCH_SIZE:-256}" \
        --max-delta "${max_delta}" \
        --device "${device}" \
        "${resume_bc[@]}"
fi
...
"${python_bin}" scripts/train_manisoft_history_mlp_ppo.py \
    --koopman-checkpoint "${koopman_checkpoint}" \
    --scenario "${scenario}" \
    --reference "${reference}" \
    --output "${ppo_output}" \
    --episode-steps "${HISTORY_MLP_EPISODE_STEPS:-300}" \
    --max-delta "${max_delta}" \
    --num-envs "${HISTORY_MLP_NUM_ENVS:-1}" \
    --actor-learning-rate "${HISTORY_MLP_ACTOR_LEARNING_RATE:-0.0001}" \
    --target-kl "${HISTORY_MLP_TARGET_KL:-0.02}" \
    --device "${device}" \
    "${ppo_initialization[@]}"
