...
def main() -> None:
    ...
    target_offset = tuple(
        float(value) for value in args.target_offset.split(",")
    )
    if len(target_offset) != 3:
        raise ValueError("--target-offset必须包含3个数字")
