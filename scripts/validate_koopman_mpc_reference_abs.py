...
using the ABSOLUTE-ACTION Koopman model ``z_{t+1}=A z_t + B u_t``.

The model state is the raw 45-D physical state ``s_t`` (no previous-action
block) and the action is the absolute 18-D muscle activation ``u_t``.
The environment is the plain ManiSoftTipTrackingEnv (45-D observation,
absolute actions), so no DeltaActionWrapper / tip-error appendage is used.
"""
...
class KoopmanMPCQP:
    """Condensed OSQP controller for ``z+ = A z + B u`` (absolute action)."""
    ...
    def __init__(
        ...
        *,
        ...
        # Action-magnitude regularizer ||u||^2 (control effort on u itself).
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Absolute-action Koopman-MPC tracking of a fixed-action "
        "reference equilibrium"
    )
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--scenario", required=True)
    parser.add_argument("--reference", required=True, help="reference.npz")
    parser.add_argument("--output", default="runs/koopman_mpc_reference_abs")
