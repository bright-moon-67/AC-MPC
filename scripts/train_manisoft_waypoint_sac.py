...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "new residual SAC policy. This mode cannot be combined with --resume."
        ),
    )
    parser.add_argument(
...
def _load_config(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.actor_anchor_coef > 0 and args.resume is None:
        raise ValueError("actor anchoring requires --resume as its source policy")
    ...
    if residual_requested and args.resume is not None:
        raise ValueError("frozen residual mode cannot be combined with --resume")
    if residual_requested and args.actor_anchor_coef > 0:
        raise ValueError("frozen residual mode cannot use actor anchoring")
    ...
    for path_key in ("entry_bank_path", "table_action_calibration_path"):
...
def main() -> None:
    ...
    if int(training["eval_freq"]) > 0:
        ...
        # Entry reweighting is a training intervention. Keep the fixed panel
        # uniform so weak-posture oversampling cannot inflate model selection.
